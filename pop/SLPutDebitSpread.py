...
def calculate_pop(percentage, closing_days, underlying, sigma, rate, trials, days_to_expiration, short_strike, short_price, long_strike, long_price):
    ...
    pop_value = float(poptions.putDebitSpread(
...
def main():
    try:
        ...
        short_strike = st.number_input("Enter the short strike:", value=0.00, placeholder="e.g. 347.50", min_value=0.00)
        short_price = st.number_input("Enter the short price:", value=0.00, placeholder="e.g. 1.01", min_value=0.00)
        long_strike = st.number_input("Enter the long strike:", value=0.00, placeholder="e.g. 350", min_value=0.00)
